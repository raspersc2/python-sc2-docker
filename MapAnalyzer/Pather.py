...
class MapAnalyzerPather:
    ...
    def __init__(self, map_data: "MapData") -> None:
        ...
        nonpathable_indices = np.where(
            self.map_data.bot.game_info.pathing_grid.data_numpy == 0
        )
    ...
    def find_eligible_point(
        ...
        if grid[point] == np.inf:
            ...
            same_height_cond = np.logical_and(
                terrain_height[disk] == target_height, grid[disk] < np.inf
            )
            ...
            if np.any(same_height_cond):
                ...
                diff_height_cond = np.logical_and(
                    terrain_height[disk] != target_height, grid[disk] < np.inf
                )
    ...
    def get_climber_grid(
        ...
        grid = np.where(
            self.map_data.c_ext_map.climber_grid != 0, default_weight, grid
        ).astype(np.float32)
        return grid
    ...
    def pathfind_pyastar(
        ...
        path = self.pyastar.astar_path(
            grid, start=start, goal=goal, allow_diagonal=allow_diagonal
        )
        if path is not None:
