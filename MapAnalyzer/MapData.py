...
class MapData:
    ...
    def __init__(
        ...
        self.c_ext_map = CMapInfo(
            pathing_grid.T, self.terrain_height.T, self.bot.game_info.playable_area
        )
    ...
    def pathfind_pyastar(
        ...
        **IF NO** ``grid`` **has been provided**, will request a fresh grid from :class:`.Pather`
        ...
        """
        return self.pather.pathfind_pyastar(
            start=start,
            goal=goal,
            grid=grid,
    ...
    def points_to_numpy_array(
        self,
        points: Union[Set[Tuple[int64, int64]], List[Point2], Set[Point2]],
        default_value: int = 1,
    ) -> ndarray:
        """
        :rtype: numpy.ndarray
    ...
    def _clean_plib_chokes(self) -> None:
        ...
        self.overlapping_choke_ids = self._get_overlapping_chokes(
            local_chokes=self.c_ext_map.chokes, areas=areas
        )
    ...
    def _calc_chokes(self) -> None:
        ...
        chokes = [
            c for c in self.c_ext_map.chokes if c.id not in self.overlapping_choke_ids
        ]
    ...
    def plot_influenced_path_pyastar(
        ...
        self.debugger.plot_influenced_path_pyastar(
            start=start,
            goal=goal,
